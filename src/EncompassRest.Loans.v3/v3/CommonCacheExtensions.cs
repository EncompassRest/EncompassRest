...
namespace EncompassRest.v3
{
    ...
    public static class CommonCacheExtensions
    {
        ...
        /// <returns></returns>
        public static ReadOnlyDictionary<string, FieldDescriptor> GetCustomFields(this CommonCache commonCache) => (ReadOnlyDictionary<string, FieldDescriptor>)commonCache.ExtensionData.GetOrAdd("v3ReadOnlyCustomFields", k => new ReadOnlyDictionary<string, FieldDescriptor>(GetCustomFieldsInternal(commonCache)));

        private static ConcurrentDictionary<string, FieldDescriptor> GetCustomFieldsInternal(CommonCache commonCache) => (ConcurrentDictionary<string, FieldDescriptor>)commonCache.ExtensionData.GetOrAdd("v3CustomFields", k => new ConcurrentDictionary<string, FieldDescriptor>(StringComparer.OrdinalIgnoreCase));
        ...
        /// <param name="commonCache">The common cache.</param>
        /// <returns></returns>
        public static DateTime? GetCustomFieldsLastRefreshedUtc(this CommonCache commonCache) => commonCache.ExtensionData.TryGetValue("v3CustomFieldsLastRefreshedUtc", out var dateTime) ? (DateTime)dateTime : null;
        ...
        public static async Task RefreshCustomFieldsAsync(this CommonCache commonCache, IEncompassRestClient client, CancellationToken cancellationToken = default)
        {
            ...
            commonCache.ExtensionData["v3CustomFieldsLastRefreshedUtc"] = DateTime.UtcNow;
        }
    }
}
