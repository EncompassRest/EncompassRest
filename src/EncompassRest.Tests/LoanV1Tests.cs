...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private sealed class FieldMappingComparer : IEqualityComparer<KeyValuePair<string, FieldDescriptorBase>>
        {
            public bool Equals(KeyValuePair<string, FieldDescriptorBase> x, KeyValuePair<string, FieldDescriptorBase> y) => x.Value._modelPath.Equals(y.Value._modelPath);

            public int GetHashCode(KeyValuePair<string, FieldDescriptorBase> obj) => obj.Value._modelPath.GetHashCode();
        }
        ...
        public void Loan_FieldsComplex()
        {
            ...
            Assert.AreEqual("Loan.Contacts[(ContactType == 'CUSTOM')][4].State", field.ModelPath, ignoreCase: true);
            Assert.IsTrue(field.IsEmpty);
            Assert.IsNull(field.Value);
            var value = "MO";
            field.Value = value;
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            Assert.AreEqual(members.Count, options.Count);
            var attribute = typeof(Borrower).GetProperty(nameof(Borrower.ApplicationTakenMethodType)).GetCustomAttribute<LoanFieldPropertyAttribute>();
            foreach (var member in members)
            {
            ...
            options = field.Descriptor.Options;
            attribute = typeof(Hmda).GetProperty(nameof(Hmda.AUS2)).GetCustomAttribute<LoanFieldPropertyAttribute>();
