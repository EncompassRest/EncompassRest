...
namespace EncompassApi
{
    ...
    {
        /// <summary>
        /// The <see cref="IEncompassApiClient"/> associated with the Api object.
        /// </summary>
        IEncompassApiClient Client { get; }
    ...
    public abstract class ApiObject : IApiObject
    {
        ...
        /// <summary>
        /// The <see cref="EncompassApiClient"/> associated with the Api object.
        /// </summary>
        public IEncompassApiClient Client { get; }

        IEncompassApiClient IApiObject.Client => Client;

        internal ApiObject(IEncompassApiClient client, string? baseApiPath)
        {
            Client = client;
        ...
        internal async Task<T> SendFullUriAsync<T>(HttpRequestMessage request, string? methodName, string? resourceId, CancellationToken cancellationToken, Func<HttpResponseMessage, Task<T>>? func, bool throwOnNonSuccessStatusCode = true, bool disposeResponse = true)
        {
            using (request)
            {
                ...
                try
                {
                    ...
                    {
                        throw await EncompassApiException.CreateAsync(CreateErrorMessage(methodName!, resourceId), response).ConfigureAwait(false);
                    }
