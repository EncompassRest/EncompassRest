...
namespace EncompassApi
{
    ...
    /// </summary>
    public sealed class EncompassApiException : Exception
    {
        internal static async Task<EncompassApiException> CreateAsync(string message, HttpResponseMessage response)
        {
            ...
            return new EncompassApiException(message, response, responseContent, requestContent, correlationId);
        }
        ...
        private EncompassApiException(string message, HttpResponseMessage response, string? responseContent, string? requestContent, string? correlationId)
