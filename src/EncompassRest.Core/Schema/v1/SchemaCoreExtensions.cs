...
namespace EncompassRest.Schema.v1
{
    ...
    /// </summary>
    public static class SchemaCoreExtensions
    {
        /// <summary>
        /// The custom v1 Api implementation for unit testing.
        /// </summary>
        public static ISchemaCoreV1? V1 { get; set; }

        private static ISchemaCoreV1 GetV1(ISchema schema)
        {
            var v1 = V1;
            if (schema is Schema s)
            {
                v1 = s.ExtensionData.GetOrAdd(() => new SchemaCoreV1(s.Client));
            }
            else if (v1 == null)
            {
