...
public sealed class LoanFieldDescriptors : ILoanFieldDescriptors
{
    ...
    internal static FieldDescriptor GetFieldDescriptor(string fieldId, IDictionary<string, FieldDescriptor>? customFields, UndefinedCustomFieldHandling undefinedCustomFieldHandling)
    {
        ...
        if (!FieldMappings.TryGetDescriptor(fieldId, out var d) && customFields?.TryGetValue(fieldId, out descriptor) != true && (!FieldPatternMappings.TryGetDescriptorForFieldId(fieldId, out d) || (d.Type == LoanFieldType.Custom && (undefinedCustomFieldHandling == UndefinedCustomFieldHandling.Error || (undefinedCustomFieldHandling == UndefinedCustomFieldHandling.ErrorIfCustomFieldsInitialized && (customFields?.Count ?? 0) != 0)))))
        {
            throw new ArgumentException($"Could not find field '{fieldId}'");
        }
        return descriptor ?? (FieldDescriptor)d!;
    }
