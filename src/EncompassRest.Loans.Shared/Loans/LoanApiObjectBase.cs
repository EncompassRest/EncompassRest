...
internal abstract class LoanApiObjectBase<T> : LoanApiObject
    where T : DirtyExtensibleObject
{
    internal readonly ILoanObjectBoundApis? LoanObjectBoundApis;

    internal LoanApiObjectBase(EncompassRestClient client, ILoanApis loanApis, string loanId, string baseApiPath)
        : base(client, loanApis, loanId, baseApiPath)
    {
        LoanObjectBoundApis = loanApis as ILoanObjectBoundApis;
    }

    internal abstract IList<T> GetInLoan(ILoan loan);
