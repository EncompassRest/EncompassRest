...
namespace EncompassRest.Loans
{
    ...
    /// </summary>
    public abstract class LoanFieldPatternMappingsBase : IDictionary<string, string>, IReadOnlyDictionary<string, string>
    {
        ...
        internal LoanFieldPatternMappingsBase()
        {
        }
        ...
        internal void AddField(FieldDescriptorBase descriptor) => (descriptor.Type == LoanFieldType.Virtual ? _virtualFieldPatterns : _standardFieldPatterns).TryAdd(descriptor.FieldId, descriptor);
...
#pragma warning restore CS8767 // Nullability of reference types in type of parameter doesn't match implicitly implemented member (possibly because of nullability attributes).
        ...
        internal bool TryGetDescriptorForFieldId(string fieldId, [NotNullWhen(true)] out FieldDescriptorBase? descriptor)
        {
        ...
        private FieldDescriptorBase CreateFieldDescriptor(string fieldPattern, string modelPathPattern)
        {
            Preconditions.NotNullOrEmpty(modelPathPattern, nameof(modelPathPattern));
            ...
            var descriptor = CreateFieldDescriptorInternal(fieldPattern, modelPathPattern);
        ...
        internal abstract FieldDescriptorBase CreateFieldDescriptorInternal(string fieldPattern, string modelPathPattern);
        ...
        internal sealed class LoanFieldPatternMappingsCollection : IEnumerable<KeyValuePair<string, FieldDescriptorBase>>
        {
            private sealed class Node
            {
                ...
                public IEnumerable<KeyValuePair<string, FieldDescriptorBase>> Enumerate(List<string> path)
                {
                    foreach (var pair in Values)
                    {
                        yield return new KeyValuePair<string, FieldDescriptorBase>(string.Concat(path) + pair.Value.InstanceSpecifier + pair.Key, pair.Value.Descriptor);
                    }
                ...
                public sealed class InstanceSpecifierAndDescriptor
                {
                    ...
                    public FieldDescriptorBase Descriptor { get; }

                    public InstanceSpecifierAndDescriptor(string instanceSpecifier, FieldDescriptorBase descriptor)
                    {
            ...
            public FieldDescriptorBase this[string fieldPattern]
            {
                get => TryGetValue(fieldPattern, out var modelPathPattern) ? modelPathPattern : throw new KeyNotFoundException();
                set
                {
            ...
            public ICollection<FieldDescriptorBase> Descriptors => this.Select(p => p.Value).ToList();
            ...
            public bool TryAdd(string fieldPattern, FieldDescriptorBase descriptor)
            {
                ValidateFieldPattern(fieldPattern, out var instanceSpecifierIndex);
                return TryAdd(fieldPattern, instanceSpecifierIndex, descriptor);
            }

            private bool TryAdd(string fieldPattern, int instanceSpecifierIndex, FieldDescriptorBase descriptor)
            {
                var endBracketIndex = fieldPattern.IndexOf('}', instanceSpecifierIndex + 2);
            ...
            public bool TryRemove(string fieldPattern, [NotNullWhen(true)] out FieldDescriptorBase? descriptor)
            {
                ValidateFieldPattern(fieldPattern, out var instanceSpecifierIndex);
                var endBracketIndex = fieldPattern.IndexOf('}', instanceSpecifierIndex + 2);
            ...
            public bool TryGetValue(string fieldPattern, [NotNullWhen(true)] out FieldDescriptorBase? descriptor)
            {
                ValidateFieldPattern(fieldPattern, out var instanceSpecifierIndex);
                var endBracketIndex = fieldPattern.IndexOf('}', instanceSpecifierIndex + 2);
            ...
            public FieldDescriptorBase GetOrAdd(string fieldPattern, FieldDescriptorBase descriptor)
            {
                ValidateFieldPattern(fieldPattern, out var instanceSpecifierIndex);
                var endBracketIndex = fieldPattern.IndexOf('}', instanceSpecifierIndex + 2);
                    ...
                    foreach (var pair in node.Values)
                    {
                        ...
                        if (instanceSpecifierLength > 1 ? fieldId.Length - start - instanceSpecifierLength >= postfix.Length : fieldId.Length - postfix.Length > start)
                        {
                            ...
                            if (i == postfix.Length)
                            {
                                ...
                                descriptor = (FieldDescriptorBase)pair.Value.Descriptor.GetInstanceDescriptor(instanceSpecifier);
...
#pragma warning restore CS8762 // Parameter may not have a null value when exiting in some condition.
            ...
            public IEnumerator<KeyValuePair<string, FieldDescriptorBase>> GetEnumerator()
            {
