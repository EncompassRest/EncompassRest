...
namespace EncompassRest.Loans.Milestones
{
    public sealed class LoanMilestones : LoanApiObject
    {
        ...
        public Task UpdateMilestoneAsync(LoanMilestone milestone, MilestoneAction action, CancellationToken cancellationToken = default) => UpdateMilestoneAsync(milestone, action.Validate(nameof(action)).AsString(), cancellationToken);
